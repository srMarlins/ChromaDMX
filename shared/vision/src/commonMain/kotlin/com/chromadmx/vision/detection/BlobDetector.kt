...
class BlobDetector(
        ...
        for (y in 0 until h) {
            for (x in 0 until w) {
                ...
                if (pixels[idx] >= brightnessThreshold && labels[idx] == 0) {
                    ...
                    floodFill(pixels, labels, w, h, x, y, label, acc, stackArray)
    ...
    private fun floodFill(
        ...
        stackArray[stackSize++] = startIdx
        labels[startIdx] = label

        while (stackSize > 0) {
            val idx = stackArray[--stackSize]
            val x = idx % w
            val y = idx / w
