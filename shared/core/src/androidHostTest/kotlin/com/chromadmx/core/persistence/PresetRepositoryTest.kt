...
class PresetRepositoryTest {
    ...
    fun insertAndQueryPreset() = runTest {
        val preset = createPreset(id = "p1", name = "Techno 1")
        repo.save(preset)
    ...
    @Test
    fun allPresetsOrderedByCreatedAt() = runTest {
        repo.save(createPreset(id = "p1", createdAt = 100L))
        repo.save(createPreset(id = "p2", createdAt = 300L))
        repo.save(createPreset(id = "p3", createdAt = 200L))

        val all = repo.allPresets().first()
        assertEquals("p2", all[0].id)
        assertEquals("p3", all[1].id)
        assertEquals("p1", all[2].id)
    }
