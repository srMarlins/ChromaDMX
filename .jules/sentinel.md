...
## 2024-05-24 - Missing KeyboardOptions for Secrets
**Vulnerability:** Passwords and API keys could be cached by the OS keyboard dictionary.
**Learning:** Compose text fields need both `PasswordVisualTransformation` and `KeyboardOptions(keyboardType = KeyboardType.Password, autoCorrectEnabled = false)` to prevent OS caching and auto-complete.
**Prevention:** Always combine visual transformation with strict keyboard options for secret inputs.
